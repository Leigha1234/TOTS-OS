...
export default function CampaignsPage() {
  ...
  const applyClarity = () => {
    ...
    setTimeout(() => {
      setForm(prev => {
        const refined = prev.content
          .replace(/\b(just|actually|really|basically|simply)\b/gi, "")
          .replace(/\b(help)\b/gi, "facilitate")
          .replace(/\b(get in touch|contact us)\b/gi, "initiate correspondence")
          .replace(/\s\s+/g, ' ')
          .trim();
        return { ...prev, content: refined };
      });
    ...
    <div className="min-h-screen bg-[#faf9f6] p-8 md:p-12 text-stone-900 font-sans">
      ...
      <header className="max-w-7xl mx-auto flex justify-between items-end mb-16 border-b border-stone-200 pb-10">
        ...
        <button 
          ...
          className="bg-[#a9b897] text-stone-800 px-10 py-5 rounded-2xl font-black text-[11px] uppercase tracking-[0.3em] flex items-center gap-3 hover:bg-[#99a986] transition-all shadow-sm"
        >
            ...
            campaigns.map(c => (
              <div key={c.id} className="bg-white p-10 rounded-[3rem] border border-stone-100 flex justify-between items-center group">
                <div className="flex items-center gap-8">
                  <div className="p-5 bg-stone-50 rounded-2xl text-[#a9b897] group-hover:bg-stone-100"><Clock size={24} /></div>
                  <div>
                ...
                <div className="text-[10px] font-black uppercase tracking-widest px-6 py-2 bg-stone-50 rounded-full text-stone-400 border border-stone-100">{c.status}</div>
              </div>
            ))
          )}
        </div>

        {/* REPAIRED DARK CARD -> NOW CONSISTENT LIGHT CARD */}
      ...
      {/* MODAL - OBSESSIVE STYLING CONSISTENCY */}
      <AnimatePresence>
        {showModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10 bg-stone-900/60 backdrop-blur-xl">
            <motion.div 
              initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
              className="bg-[#faf9f6] w-full max-w-[1400px] h-full rounded-[4rem] shadow-2xl flex flex-col md:flex-row overflow-hidden relative"
            >
              {/* SIDEBAR NAVIGATION */}
              <div className="w-80 bg-stone-50 border-r border-stone-200 p-12 flex flex-col shrink-0">
                ...
                <div className="space-y-4 mb-12">
                  {TEMPLATES.map(t => (
                    <button 
                      ...
                      className={`w-full flex items-center gap-4 p-5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${form.template_id === t.id ? 'bg-[#a9b897] text-stone-800 shadow-md' : 'bg-white text-stone-400 border border-stone-100 hover:border-stone-300'}`}
                    >
                      {t.icon} {t.name}
                    </button>
                  ))}
                ...
                <div className="mt-auto">
                  <button 
                    ...
                    className="w-full bg-[#e2e8d8] text-stone-800 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-[#d4dfc7] transition-all"
                  >
                    Cancel
                  </button>
                </div>
              ...
              {/* EDITOR CANVAS */}
              <div className="flex-1 bg-stone-100/50 p-8 md:p-16 overflow-y-auto flex flex-col items-center">
                <div className="w-full max-w-3xl flex justify-between items-center mb-10">
                   <div className="flex items-center gap-3">
                     ...
                     <span className="text-[9px] font-black uppercase tracking-widest text-stone-400">Live Draft</span>
                   </div>
                  <button 
                    onClick={applyClarity}
                    className={`px-8 py-3 rounded-full shadow-md text-[10px] font-black uppercase tracking-widest flex items-center gap-3 transition-all ${didEnhance ? 'bg-green-100 text-green-700' : 'bg-[#a9b897] text-stone-800 hover:shadow-lg'}`}
                  >
                    {isEnhancing ? <Loader2 size={14} className="animate-spin" /> : didEnhance ? <Check size={14} /> : <Wand2 size={14} />} 
                    {didEnhance ? "Clarity Applied" : "Clarity Engine"}
                  </button>
                </div>
                ...
                <div className="bg-white w-full max-w-3xl min-h-[900px] rounded-[4rem] border border-stone-200 p-24 flex flex-col shadow-xl">
                  ...
                  <footer className="mt-20 pt-10 border-t border-stone-50 flex flex-col items-center gap-3">
                     ...
                     <p className="text-[10px] text-stone-300 uppercase tracking-widest italic">London — Studio 4</p>
                  </footer>
                </div>

                <div className="mt-16 pb-12">
                  <button 
                    className="bg-stone-900 text-[#a9b897] px-28 py-7 rounded-[2.5rem] font-black text-xs uppercase tracking-[0.5em] shadow-2xl hover:scale-105 transition-transform"
                  >
                    Schedule Dispatch
                  </button>
                </div>
