...
import {
  BrainCircuit, Plus, Hash, Sparkles, Command,
  Users, Share2, Send, AlertCircle
} from "lucide-react";

/**
 * TOTS OS v5.2 - CLARITY PROTOCOL
 * FULL FUNCTIONAL WORKSPACE: CHATS, TEAM, & ACCOUNT-AWARE AI
 */
...
export default function ClarityPage() {
  ...
  const [allSignals, setAllSignals] = useState<Signal[]>([
    {
      ...
      role: "assistant",
      content:
        "Clarity initialized. I can see your live revenue, tasks, hours, social, and email data — ask me anything about your account.",
      timestamp: new Date(),
    },
  ]);
  ...
  // --- LOGIC: ACCOUNT-AWARE QUERY ENGINE ---
  const handleQuery = async (e: React.FormEvent) => {
    ...
    // Snapshot the conversation history for this workspace BEFORE adding the
    // new user turn, so the API gets prior context without duplicating it.
    const historyForApi = currentFeed
      .filter((s) => !s.isError)
    ...
    history: historyForApi,
      ...
      setAllSignals((prev) => [
        ...prev,
        {
          projectId: activeProjectId,
          role: "assistant",
          content: payload.answer || "I couldn't generate a response.",
          timestamp: new Date(),
        },
      ]);
    } catch (err) {
        ...
        ...prev,
        {
          ...
          content:
            err instanceof Error
              ...
              : "Something went wrong reaching Clarity. Please try again.",
          timestamp: new Date(),
          isError: true,
        },
      ]);
          ...
          <div className="flex -space-x-2 mb-4">
            {teamMembers.map((m) => (
              <div
                key={m.id}
                title={m.name}
                className="w-8 h-8 rounded-full border-2 border-white bg-stone-100 flex items-center justify-center text-[10px] font-black text-stone-500"
              >
                {m.initials}
              </div>
            ))}
            <button
              ...
              className="w-8 h-8 rounded-full border-2 border-white bg-stone-900 flex items-center justify-center text-[#A9B897] hover:scale-105 transition-transform"
            >
              <Plus size={12} />
            </button>
          </div>
            ...
            {currentFeed.length === 0 && (
              <div className="pt-20 text-center space-y-4">
                ...
                <div className="flex flex-wrap justify-center gap-3">
                  {["What's my outstanding revenue?", "Which invoices are overdue?", "How are my email campaigns performing?"].map(
                    (suggestion) => (
                      <button
                        key={suggestion}
                        onClick={() => setQuery(suggestion)}
                        className="px-4 py-2 bg-stone-50 border border-stone-100 rounded-full text-[10px] font-black uppercase tracking-widest text-stone-400 hover:border-[#A9B897] hover:text-[#A9B897] transition-all"
                      >
                        {suggestion}
                      </button>
                    )
            ...
            <AnimatePresence mode="popLayout">
              {currentFeed.map((signal, i) => (
                <motion.div
                  ...
                  className={`flex gap-6 ${signal.role === "user" ? "flex-row-reverse" : ""}`}
                >
                  <div
                  ...
                  >
                    {signal.role === "user" ? (
                      <div className="text-[10px] font-black">LD</div>
                    ) : signal.isError ? (
                      <AlertCircle size={14} />
                    ) : (
                      <Sparkles size={14} />
                      ...
                      className={`text-[15px] leading-[1.7] whitespace-pre-wrap ${
                        signal.role === "assistant"
                          ? signal.isError
                            ...
                            : "font-serif italic text-lg text-stone-800"
          ...
          <div className="max-w-3xl mx-auto group">
            <form onSubmit={handleQuery} className="relative bg-white border border-stone-200 shadow-2xl rounded-[2.5rem] p-2 flex items-center">
              ...
              <input
                ...
                placeholder="Ask Clarity about your account..."
                disabled={isAiTyping}
                className="flex-1 bg-transparent py-5 text-sm outline-none font-medium placeholder:text-stone-300 disabled:opacity-50"
              />
              <button
                type="submit"
                disabled={isAiTyping || !query.trim()}
                className="bg-stone-900 text-[#A9B897] p-3 rounded-full hover:scale-105 transition-all ml-2 disabled:opacity-30 disabled:pointer-events-none"
              >
                <Send size={20} />
              </button>
            </form>
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full border-b border-stone-100 py-2 outline-none focus:border-stone-900 font-serif italic text-xl transition-all"
                  />
                </div>
                <button onClick={createChat} className="w-full bg-stone-900 text-[#A9B897] py-5 rounded-2xl text-[10px] font-black uppercase tracking-widest">
                  Establish Node
                </button>
                <button onClick={() => setShowCreator(false)} className="w-full text-stone-400 text-[10px] font-black uppercase tracking-widest">
                  Cancel
                </button>
              </div>
            </motion.div>
          ...
          <div className="fixed inset-0 bg-stone-950/40 backdrop-blur-md z-[100] flex items-center justify-center p-6">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white w-full max-w-md rounded-[3rem] p-10 shadow-2xl border border-stone-100 text-center">
              ...
              <div className="space-y-4">
                ...
                <button onClick={() => setShowInvite(false)} className="w-full bg-stone-900 text-[#A9B897] py-5 rounded-2xl text-[10px] font-black uppercase tracking-widest">
                  Close
                </button>
              </div>
            </motion.div>
