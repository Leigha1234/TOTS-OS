  ...
  async function calculateFinancials() {
    try {
      ...
      setMetrics({
        ...
        netMargin: billable - taxAmount - 12400
      });
    ...
    <AnimatePresence>
      {activeModal === id && (
        ...
        >
          ...
          >
            <button onClick={() => setActiveModal(null)} className="absolute top-12 right-12 p-5 bg-stone-50 rounded-full hover:bg-stone-900 hover:text-white transition-all text-stone-400 group">
    ...
    <div className="min-h-screen bg-[#fcfbf9] text-stone-900 font-sans selection:bg-[#a9b897] selection:text-white overflow-x-hidden">
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-12 md:p-20 space-y-24">
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-6">
              ...
              <div className="space-y-1">
                <p className="font-black uppercase text-[10px] tracking-[0.5em] text-stone-400">Status: Intelligence Engine Active</p>
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-[#a9b897] animate-pulse" />
                  <p className="text-[9px] font-mono text-stone-400 tracking-widest">NODE_V5.2.4_SYNCED</p>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-8 bg-white border border-stone-100 rounded-[5rem] p-12 md:p-20 space-y-16 shadow-sm relative group overflow-hidden">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-10 relative z-10">
              ...
              <div className="flex bg-stone-50 p-2.5 rounded-[2.5rem] border border-stone-100 shadow-inner">
                {['6M', '1Y', 'ALL'].map(t => (
                  <button key={t} className={`px-12 py-4 rounded-[2rem] text-[10px] font-black tracking-[0.3em] transition-all uppercase ${t === '1Y' ? 'bg-stone-900 text-white shadow-xl scale-105' : 'text-stone-400 hover:text-stone-900'}`}>{t}</button>
                ))}
              </div>
            </div>
            ...
            <div className="h-[450px] flex items-end gap-4 md:gap-10 px-4 relative z-10">
              {[40, 55, 30, 75, 65, 90, 85, 50, 95, 100, 70, 80].map((val, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-8 group/bar h-full justify-end">
                  ...
                  >
                    <div className="absolute -top-16 left-1/2 -translate-x-1/2 opacity-0 group-hover/bar:opacity-100 transition-all bg-stone-900 text-white text-[10px] px-6 py-4 rounded-2xl shadow-2xl whitespace-nowrap z-20 font-mono font-bold tracking-tighter">£{val}k</div>
                  </motion.div>
              ...
              <div className="space-y-6">
                {[
                  ...
                  <button 
                    ...
                    className="w-full flex items-center justify-between p-12 bg-white/5 border border-white/10 rounded-[3.5rem] hover:bg-white/10 transition-all group/btn active:scale-95"
                  >
                    <div className="flex items-center gap-8">
                      <div className="w-16 h-16 rounded-[1.5rem] bg-white/5 flex items-center justify-center group-hover/btn:bg-[#a9b897]/20 group-hover/btn:text-[#a9b897] transition-all shadow-inner">
                        <btn.icon size={24} />
                      </div>
        ...
        <section className="bg-white border border-stone-100 rounded-[5rem] overflow-hidden shadow-sm hover:border-stone-200 transition-all duration-1000">
           <div className="p-16 md:p-24 border-b border-stone-50 flex flex-col lg:flex-row items-center justify-between gap-12">
              <div className="flex items-center gap-12 text-center lg:text-left">
              ...
              <button 
                ...
                className="bg-stone-50 px-12 py-6 rounded-[2.5rem] text-[11px] font-black uppercase tracking-[0.3em] text-stone-400 hover:text-stone-900 hover:bg-white border border-transparent hover:border-stone-200 flex items-center gap-5 transition-all shadow-sm"
              >
              ...
              <tbody className="divide-y divide-stone-50">
                {[
                  ...
                  <tr key={i} className="group hover:bg-stone-50/40 transition-all cursor-pointer">
                    ...
                    <td className="px-20 py-16">
                      <span className={`px-10 py-4 text-[10px] font-black uppercase tracking-[0.4em] rounded-full border shadow-sm transition-all ${row.status === 'In Review' ? 'bg-white text-stone-400 border-stone-100 italic' : 'bg-stone-900 text-white border-stone-900 group-hover:bg-[#a9b897] group-hover:border-[#a9b897]'}`}>{row.status}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <Modal id="audit" title="System Audit Record" subtitle="Node Infrastructure Scan">
        <div className="space-y-16 py-10">
          <div className="bg-stone-50 p-12 rounded-[4rem] border border-stone-100 space-y-10 shadow-inner">
            <div className="flex justify-between items-center border-b border-stone-200 pb-10">
          ...
          <div className="grid grid-cols-2 gap-10">
             ...
             <button className="flex items-center justify-center gap-6 py-10 bg-stone-50 text-stone-400 border border-stone-100 rounded-[2.5rem] text-[11px] font-black uppercase tracking-[0.5em] hover:text-stone-900 transition-all shadow-sm">
