...
import { 
  ...
  Fingerprint, Mail, Send, FileCheck
} from "lucide-react";
      ...
      <AnimatePresence>
        {isNotificationVisible && (
          <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} className="fixed bottom-12 left-1/2 -translate-x-1/2 z-[200] bg-stone-900 text-white px-10 py-5 rounded-full shadow-2xl flex items-center gap-4">
            <Send size={14} className="text-[#a9b897]" />
      ...
      <div className="max-w-[1400px] mx-auto space-y-12">
        ...
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-8">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <Plus size={18} />
              <span className="text-[9px] font-black uppercase tracking-widest">New Document</span>
            </button>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          <div className="bg-stone-900 text-white p-10 rounded-[4rem] shadow-2xl flex flex-col justify-between min-h-[300px] relative overflow-hidden group">
            <div className="z-10 space-y-4">
              <div className="flex justify-between items-start">
                <p className="text-[10px] font-black uppercase tracking-widest text-stone-500">Gross Revenue</p>
                <div className="bg-[#a9b897]/20 p-2 rounded-lg"><ArrowUpRight size={18} className="text-[#a9b897]" /></div>
              </div>
              <h2 className="text-5xl font-mono tracking-tighter pt-4 leading-none truncate">£{metrics.revYtd.toLocaleString()}</h2>
            </div>
            <div className="z-10 bg-white/5 border border-white/10 rounded-2xl p-4 flex items-center justify-between">
              <span className="text-[8px] font-black uppercase tracking-[0.2em] text-stone-500">Live Ledger Feed</span>
          ...
          {[
            ...
            { label: 'Corp Tax Due', value: metrics.taxDue, sub: 'Projected End YTD', action: 'Reports', icon: <Receipt size={18}/> }
          ].map((item, i) => (
        ...
        <section className="bg-white border border-stone-100 rounded-[4rem] overflow-hidden shadow-sm">
          <div className="p-10 md:p-14 flex flex-col md:flex-row justify-between items-center gap-8 border-b border-stone-50">
            <div className="space-y-1 text-center md:text-left">
              <h3 className="text-4xl font-serif italic tracking-tighter">Transaction Ledger</h3>
              <p className="text-[9px] font-black uppercase tracking-[0.4em] text-stone-400">Audited Financial History</p>
            </div>
            ...
            <table className="w-full text-left min-w-[900px]">
              <thead>
                <tr className="bg-stone-50/50">
                  {['Reference', 'Entity', 'Type', 'Status', 'Amount'].map((h) => (
                    <th key={h} className="px-14 py-6 text-[9px] font-black uppercase tracking-[0.3em] text-stone-400">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-stone-50">
                {isLoading ? (
                  ...
                  <tr key={inv.id} className="group hover:bg-stone-50/20 transition-colors cursor-pointer">
                    ...
                    <td className="px-14 py-10 text-[9px] font-black uppercase text-stone-400 tracking-widest">{inv.type}</td>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
           ...
           <button onClick={() => setActiveModal('invoice')} className="p-10 md:p-14 bg-stone-900 text-white rounded-[4rem] flex items-center justify-between group hover:bg-[#a9b897] transition-all shadow-2xl">
              <div className="space-y-2 text-left">
                <p className="text-[10px] font-black uppercase text-white/20 tracking-widest">Client Distribution</p>
                <h5 className="text-4xl font-serif italic tracking-tighter">Dispatch Document</h5>
              </div>
              <div className="p-8 bg-white/10 rounded-[2rem] group-hover:bg-white group-hover:text-stone-900 transition-all"><Mail size={28} /></div>
           </button>
        </div>
        ...
        <Modal id="invoice" title="Sales Directive">
          <div className="space-y-10 py-6">
            {/* Type Selector */}
            <div className="flex bg-stone-50 p-2 rounded-2xl border border-stone-100 max-w-fit">
              {["Invoice", "Quote"].map((t) => (
                <button 
                  key={t}
                  onClick={() => setDocType(t as any)}
                  className={`px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${docType === t ? 'bg-stone-900 text-white shadow-lg' : 'text-stone-400'}`}
                >
                  {t}
                </button>
              ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4">
                ...
                <input placeholder="Client / Company Name" className="w-full p-6 bg-stone-50 border border-stone-100 rounded-2xl outline-none focus:border-stone-900 focus:bg-white transition-all font-bold shadow-inner" />
              </div>
              <div className="space-y-4">
                <label className="text-[10px] font-black uppercase text-stone-400 ml-4 tracking-widest">Dispatch Email</label>
                <input type="email" placeholder="finance@entity.com" className="w-full p-6 bg-stone-50 border border-stone-100 rounded-2xl outline-none focus:border-stone-900 focus:bg-white transition-all font-bold shadow-inner" />
              </div>
              <div className="space-y-4">
              ...
              <div className="space-y-4">
                <label className="text-[10px] font-black uppercase text-stone-400 ml-4 tracking-widest">Reference Code</label>
                <input placeholder="AUTO-GENERATE" disabled className="w-full p-6 bg-stone-100 border border-stone-200 rounded-2xl text-stone-400 font-mono font-bold shadow-inner cursor-not-allowed" />
              </div>
            </div>

            <div className="space-y-4">
              <label className="text-[10px] font-black uppercase text-stone-400 ml-4 tracking-widest">Service Breakdown / Descriptions</label>
              <textarea 
                rows={4}
                placeholder="Include project scope, line items, or specific service terms..." 
                className="w-full p-8 bg-stone-50 border border-stone-100 rounded-[2.5rem] outline-none focus:border-stone-900 focus:bg-white transition-all font-medium text-sm shadow-inner leading-relaxed"
              />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
              <button 
                onClick={() => { notify(`${docType} Saved to Ledger`); setActiveModal(null); }} 
                className="w-full bg-white border border-stone-200 text-stone-900 py-6 rounded-2xl text-[10px] font-black uppercase tracking-[0.4em] hover:bg-stone-50 transition-all flex items-center justify-center gap-3"
              >
                <FileCheck size={16} /> Save Draft
              </button>
              <button 
                onClick={() => { notify(`${docType} Dispatched Successfully`); setActiveModal(null); }} 
                className="w-full bg-stone-900 text-white py-6 rounded-2xl text-[10px] font-black uppercase tracking-[0.4em] shadow-2xl hover:bg-[#a9b897] transition-all flex items-center justify-center gap-3"
              >
                Execute & Email <Send size={14} />
              </button>
            </div>
          </div>
        </Modal>
