  ...
  return (
    <div className="min-h-screen bg-[#FBFBFA] text-[#1c1c1c] p-8 font-sans antialiased">
      ...
      <nav className="max-w-7xl mx-auto flex justify-between items-center mb-16 bg-white/80 p-4 rounded-full border border-stone-100 shadow-sm backdrop-blur-xl sticky top-8 z-40">
        ...
        <button onClick={syncPosts} className="p-3 bg-stone-50 rounded-full hover:bg-stone-100 transition-colors mr-1"><RefreshCcw size={14}/></button>
      </nav>
          ...
          {activeTab === "lab" && (
            <motion.div key="lab" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-12 gap-12">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2">
                    ...
                    <div className="p-12 space-y-8">
                       <div className="space-y-4">
                        <label className="text-[10px] font-black uppercase text-stone-300">Write Post</label>
                        <textarea 
                          value={caption} 
                    ...
                    >
                      {mediaPreview ? (
                        ...
                        <div className="text-center">
                          ...
                          <p className="text-[9px] font-black uppercase tracking-[0.2em] text-stone-300">Add Media</p>
                        </div>
                      )}
              ...
              <div className="col-span-12 lg:col-span-4 pt-28 space-y-10">
                <h3 className="text-[10px] font-black uppercase text-stone-300 tracking-widest pl-2">Recent Posts</h3>
                <div className="space-y-4">
                  {posts.slice(0, 4).map(post => (
                    <div key={post.id} className="bg-white p-4 rounded-[2rem] border border-stone-100 flex items-center gap-4 shadow-sm group">
                      <div className="w-14 h-14 bg-stone-50 rounded-2xl overflow-hidden shadow-inner">
                        <img src={post.media_url} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[8px] font-black uppercase text-[#a9b897]">{post.platform}</p>
                        <p className="text-sm font-serif italic truncate text-stone-600">"{post.caption}"</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            >
              <div className="p-12 space-y-10">
                ...
                <div className="space-y-8">
                  {/* Platform */}
                  <div className="space-y-4">
                    <label className="text-[10px] font-black uppercase text-stone-300">Select Platform</label>
                    <div className="flex gap-2">
                  ...
                  {/* Scheduling */}
                  <div className="space-y-4">
                    <label className="text-[10px] font-black uppercase text-stone-300 flex items-center gap-2"><Clock size={12}/> Choose Date & Time</label>
                    <input 
                      type="datetime-local" 
                      value={scheduledDate} 
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <button onClick={generateAIHashtags} className="text-[9px] font-black uppercase text-[#a9b897] flex items-center gap-2 hover:opacity-70 transition-opacity"><Sparkles size={12}/> AI Suggester</button>
                    </div>
                    <input 
                      value={hashtags} 
