...
export default function SocialPage() {
  const [activeTab, setActiveTab] = useState<"lab" | "horizon">("lab");

  // Lab Tab States
  const [activeSubTab, setActiveSubTab] = useState<"synthesizer" | "drafts">("synthesizer");
    ...
    <div className="min-h-screen bg-[#faf9f6] text-stone-900 p-6 md:p-12 max-w-[1700px] mx-auto space-y-16">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-stone-200 pb-12">
        <div className="space-y-2">
          ...
          <h1 className="text-7xl font-serif italic tracking-tighter">Social Lab & Horizon</h1>
          <p className="text-stone-400 italic font-serif text-lg">Central dashboard and distribution engine.</p>
        </div>

        <div className="flex items-center gap-3 bg-white border border-stone-100 rounded-2xl p-2 shadow-sm">
          <button 
            onClick={() => setActiveTab("lab")}
            className={`px-6 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all cursor-pointer ${
              activeTab === "lab" 
          ...
          {viewMode === 'stream' ? (
            <div className="space-y-8">
              ...
              {horizonPosts.length === 0 && (
                <div className="text-center py-24 bg-stone-50 border-2 border-dashed border-stone-200 rounded-[2.5rem]">
