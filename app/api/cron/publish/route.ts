...
function getTikTokPullMediaUrl(
  sourceUrl: string
) {
...
async function createNotification({
  ...
  try {
    ...
    console.warn(
      "[NOTIFICATIONS] Primary insert failed:",
      error
    );
  }
  ...
  try {
    ...
    if (
      ...
      console.warn(
        "[NOTIFICATIONS] Legacy insert failed:",
        error
      );
    }
  } catch (
...
async function publishFacebookSinglePhoto({
  ...
  const endpoint =
    `https://graph.facebook.com/${getMetaGraphVersion()}/${connection.page_id}/photos`;
...
async function publishFacebookMultiPhoto({
  ...
  for (
    ...
    if (
      ...
      throw new Error(
        `Facebook could not upload image ${index + 1}: ${getMetaErrorMessage(
          result
        )}`
      );
    }
...
async function createInstagramCarouselChild({
  ...
  if (
    ...
    throw new Error(
      `Instagram could not prepare carousel item ${index + 1}: ${getMetaErrorMessage(
        data
      )}`
    );
  }
...
function isPermanentTikTokCode(
  code: string
) {
  return new Set([
...
async function getTikTokConnection({
  ...
  if (
    ...
    throw new PermanentPublishError(
      "TikTok account is not connected. Connect TikTok in Settings."
    );
  }
...
async function refreshTikTokConnection({
  ...
  if (
    ...
    throw new PermanentPublishError(
      "TikTok access has expired. Reconnect TikTok in Settings."
    );
  }
  ...
  if (
    ...
    throw new PermanentPublishError(
      `TikTok token refresh failed. Reconnect TikTok in Settings. ${
        cleanString(
          data
            ?.error_description
        ) ||
  ...
  if (
    ...
    throw new Error(
      `TikTok token refreshed but could not be saved: ${error.message}`
    );
  }
...
function validateTikTokSettings({
  ...
  if (
    ...
    throw new PermanentPublishError(
      "TikTok privacy has not been selected. Edit this post and choose who can view it.",
  ...
  if (
    ...
    throw new PermanentPublishError(
      "Your TikTok privacy options have changed. Edit the post and choose a currently available privacy setting.",
  ...
  if (
    ...
    throw new PermanentPublishError(
      "TikTok publishing consent was not confirmed for this post. Edit the post and confirm the TikTok declaration.",
      "missing_tiktok_consent"
    );
  }
  ...
  if (
    ...
    throw new PermanentPublishError(
      "TikTok commercial content is enabled but neither Your brand nor Branded content was selected.",
  ...
  if (
    ...
    throw new PermanentPublishError(
      "TikTok branded content cannot use Only me visibility. Edit the post and choose an allowed visibility.",
      "branded_content_private"
    );
  }

  const creatorCommentDisabled =
    Boolean(
      creatorData
        ?.data
        ?.comment_disabled
...
async function publishTikTokPost({
  ...
  if (
    ...
    throw new PermanentPublishError(
      "TikTok posts cannot mix images and videos."
    );
  }
  ...
  if (
    ...
    throw new PermanentPublishError(
      "TikTok supports one video per post in TOTS-OS."
    );
  }
  ...
  if (
    ...
    throw new PermanentPublishError(
      "TikTok access token is unavailable. Reconnect TikTok in Settings."
    );
  }
...
async function publishTikTokPhotos({
  ...
  accessToken: string;
  creatorData: any;
  settings: TikTokValidatedSettings;
  mediaUrls: string[];
  fullMessage: string;
}) {
  if (
  ...
  // TikTok photo Direct Post requires PULL_FROM_URL media to
  // use URLs under a domain or prefix verified with TikTok.
  //
  // The raw storage URL remains stored in TOTS-OS. TikTok receives
  // the verified TOTS-OS proxy URL.
      ...
      "https://open.tiktokapis.com/v2/post/publish/content/init/",
      {
        ...
        body:
          JSON.stringify({
            ...
            // TikTok's photo endpoint expects is_aigc at
            // the top level, not inside post_info.
            is_aigc:
              settings
                .isAigc,
          }),
...
async function checkTikTokPostStatus({
  ...
  if (
    ...
    /*
     * TikTok has historically exposed this field with
     * inconsistent spelling in examples/responses, so support
     * both forms while keeping publish_id as the fallback.
     */
    const postIds =
      data
        ?.data
...
export async function GET(
  ...
  const {
    ...
    await supabase
      ...
      .or(
        `attempts.lt.${MAX_ATTEMPTS},attempts.is.null`
      )
      .order(
        "scheduled_for",
        {
  ...
  // ==========================================================
  // TIKTOK POSTS WAITING FOR PROCESSING COMPLETION
  // ==========================================================
