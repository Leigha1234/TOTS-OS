...
import { 
  ...
  Clock, Zap, Plus, Filter, MoreHorizontal,
  Landmark 
} from "lucide-react";
...
export default function CalendarPage() {
  ...
  async function fetchEvents() {
    ...
    // ✅ Using getSession to prevent the "Lock Stolen" ReferenceError
  ...
  return (
    <div className="min-h-screen bg-[#faf9f6] p-8 md:p-12">
      ...
      {/* HEADER: NAVIGATION */}
      <div className="max-w-[1600px] mx-auto flex flex-col md:flex-row justify-between items-end mb-12 gap-8">
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex gap-2 mb-2">
              <button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="p-3 bg-white rounded-full border border-stone-100 hover:bg-stone-50 transition shadow-sm">
                <ChevronLeft size={20} className="text-stone-400" />
              </button>
              <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="p-3 bg-white rounded-full border border-stone-100 hover:bg-stone-50 transition shadow-sm">
        ...
        <div className="bg-[#1c1c1c] text-white p-6 rounded-[2.5rem] flex items-center gap-8 shadow-2xl border border-stone-800 min-w-[400px]">
      ...
      <div className="max-w-[1600px] mx-auto grid lg:grid-cols-12 gap-8">
            ...
            {calendarDays.map((day, i) => {
              ...
              return (
                <div 
                  ...
                  className={`min-h-[140px] p-4 border-r border-b border-stone-50 transition-all cursor-pointer hover:bg-stone-50/50 group
                    ${!isCurrentMonth ? 'bg-stone-50/30' : 'bg-white'}
                  `}
                >
                  <div className="flex justify-between items-start mb-2">
                    <span className={`text-sm font-bold ${isToday ? 'bg-stone-900 text-white w-7 h-7 flex items-center justify-center rounded-full' : isCurrentMonth ? 'text-stone-800' : 'text-stone-200'}`}>
                      {format(day, "d")}
                    </span>
                    {dayTasks.length > 0 && (
                  ...
                  <div className="space-y-1">
                    {dayTasks.slice(0, 3).map((task, idx) => (
                      <div key={idx} className="bg-stone-100 px-2 py-1 rounded-md text-[9px] font-black uppercase text-stone-500 truncate tracking-tighter">
                        {task.title}
                      </div>
                    ))}
                  </div>
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="bg-white p-8 rounded-[3rem] border border-stone-100 shadow-sm">
              ...
              {getTasksForDay(selectedDay).length === 0 ? (
                ...
                getTasksForDay(selectedDay).map((task) => (
                  <motion.div 
                    ...
                    key={task.id} 
                    className="p-4 bg-stone-50 rounded-2xl border border-stone-100 group"
                  >
            ...
            <button className="w-full mt-8 bg-stone-900 text-[#a9b897] py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl flex items-center justify-center gap-2 hover:scale-[1.02] transition">
          ...
          <div className="bg-[#a9b897] p-8 rounded-[3rem] text-white shadow-xl relative overflow-hidden group">
            <div className="relative z-10">
               ...
               <p className="text-xs mt-2 opacity-80">VAT Quarter Return due in 52 days.</p>
            </div>
